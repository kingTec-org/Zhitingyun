...
public class SetNewPwdViewModel extends BaseViewModel {
    ...
    public SetNewPwdViewModel(@NonNull Application application, Repository repository) {
        super(application);
    ...
    public void updatePwd(String phone, String code, BaseObserver<Model0<LoginEntity>> observer) {
        if (TextUtils.isEmpty(mPhone.get())) {
            ToastUtils.showShort("手机号不能为空");
            return;
        }
        ...
        mTip.set("修改密码中");
        setLoadingState(true);
        mRepository.updatePwd(phone, mPwd.get(), code, observer);
    }
