...
public class GlideUtil {
    ...
    //默认加载
    public static void loadWithPlaceHolder(Context mContext, String path, ImageView mImageView) {
        if (TextUtils.isEmpty(path)) {
            loadDefault(mContext, path, mImageView);
            return;
        }
        GlideApp.with(mContext).load(path).transition(DrawableTransitionOptions.withCrossFade())
                .placeholder(R.mipmap.gray_ic).error(R.mipmap.gray_ic)
                .into(mImageView);
    }

    //加载默认背景,路径为空不设置的话item复用 item图片有问题
    public static void loadDefault(Context mContext, String path, ImageView mImageView) {
        GlideApp.with(mContext).load(R.mipmap.gray_ic)
                .transition(DrawableTransitionOptions.withCrossFade()).into(mImageView);
    }

    //加载默认背景,路径为空不设置的话item复用 item图片有问题
    public static void loadIcDefault(Context mContext, String path, ImageView mImageView) {
        GlideApp.with(mContext).load(R.mipmap.round_gray_ic)
//                .transition(DrawableTransitionOptions.withCrossFade())
                .into(mImageView);
    }

    //加载头像设置占位图
    public static void loadIc(Context mContext, String path, ImageView mImageView) {
        if (TextUtils.isEmpty(path)) {
...
//                .transition(DrawableTransitionOptions.withCrossFade())
    ...
    //默认加载
    public static void loadCover(final Context mContext, String path, final ImageView mImageView) {
        if (TextUtils.isEmpty(path)) {
//            loadDefault(mContext, path, mImageView);
            return;
        }
//        MediaUtils.getImageForVideo(mContext, path, new MediaUtils.OnLoadVideoImageListener() {
//            @Override
...
//                .into(mImageView);
    ...
    public static void loadWithBorder(Context mContext, String path, ImageView mImageView) {
        if (TextUtils.isEmpty(path)) {
            return;
        }
        GlideApp.with(mContext).load(path).transition(DrawableTransitionOptions.withCrossFade())
    ...
    //居中显示
    public static void loadCenterCrop(Context mContext, String path, ImageView mImageView) {
        if (TextUtils.isEmpty(path)) {
            loadDefault(mContext, path, mImageView);
            return;
        }
    ...
    public static Bitmap thumbFromCache(String url, Context mContext) {
        ...
        try {
            ...
            Bitmap thumb = ImageUtils.compressByQuality(bitmap, 24 * 1024, true);
            return thumb;
